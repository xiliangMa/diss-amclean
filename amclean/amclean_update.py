...
def updateURL():
    return os.environ.get('AMCLEAN_UPDATE_URL', 'https://111.229.167.6:8080/v1/probe/virus/db/file/')
    ...
    for f in json.loads(r.text):
        ...
        try:
            with open(filename + '.tmp', "xb") as target:
                ...
                r = requests.get(updateURL() + 'download?filename=' + f['file_name'], stream=True, verify=False)
                #r = requests.get(updateURL() + f['file_name'], timeout=10, stream=True, verify=False)
                r.raise_for_status()
